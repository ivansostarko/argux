...
const I = {
    ...
    code: <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="4,5 1,8 4,11"/><polyline points="12,5 15,8 12,11"/><line x1="9.5" y1="3" x2="6.5" y2="13"/></svg>,
...
const sections: NavSection[] = [
    {
        title: 'Command',
        items: [
            { label: 'Tactical Map', icon: I.map, route: '/map' },
            { label: 'Vision', icon: I.eye, route: '/vision' },
            { label: 'Operations', icon: I.target, route: '/operations' },
        ],
    },
    {
        title: 'Subjects',
        items: [
            { label: 'Persons', icon: I.users, route: '/persons' },
            { label: 'Organizations', icon: I.building, route: '/organizations' },
            { label: 'Vehicles', icon: I.car, route: '/vehicles' },
            { label: 'Devices', icon: I.cpu, route: '/devices' },
        ],
    },
    {
        title: 'Intelligence',
        items: [
            { label: 'Plate Recognition', icon: I.plate, route: '/plate-recognition' },
            { label: 'Face Recognition', icon: I.face, route: '/face-recognition' },
            { label: 'Social Scraper', icon: I.share, route: '/scraper' },
            { label: 'Web Scraper', icon: I.globe, route: '/web-scraper' },
            { label: 'Apps', icon: I.phone, route: '/apps' },
        ],
    },
    {
        title: 'Analysis',
        items: [
            { label: 'Connections', icon: I.graph, route: '/connections' },
            { label: 'Workflows', icon: I.workflow, route: '/workflows' },
            { label: 'Data Sources', icon: I.database, route: '/data-sources' },
        ],
    },
    {
        title: 'Monitoring',
        items: [
            { label: 'Alert Rules', icon: I.alertTriangle, route: '/alerts' },
            { label: 'Activity Log', icon: I.activity, route: '/activity' },
            { label: 'Notifications', icon: I.bell, route: '/notifications' },
            { label: 'Risks', icon: I.shield, route: '/risks' },
        ],
    },
    {
        title: 'Tools',
        items: [
            { label: 'AI Assistant', icon: I.chat, route: '/chat' },
            { label: 'Records', icon: I.fileText, route: '/records' },
            { label: 'Storage', icon: I.folder, route: '/storage' },
            { label: 'Reports', icon: I.report, route: '/reports' },
        ],
    },
    {
        title: 'System',
        items: [
            { label: 'Jobs', icon: I.server, route: '/jobs' },
        ],
    },
];
...
export default function Sidebar({ collapsed, onToggle, currentPath, mobileOpen, onMobileClose }: SidebarProps) {
    ...
    const content = (
        <div style={{
            width: sidebarWidth, height: '100vh', background: '#0a0e16',
            ...
            {/* Logo */}
            <div style={{
                padding: collapsed ? '16px 8px' : '16px 16px', display: 'flex',
                alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between',
                borderBottom: `1px solid ${theme.border}`, minHeight: 56,
            }}>
                    ...
                    <div key={section.title} style={{ marginBottom: 4 }}>
                        {!collapsed && (
                            <div style={{
                                ...
                                color: theme.textDim, letterSpacing: '0.12em',
                        ...
                        {section.items.map((item) => {
                            ...
                            return (
                                <button key={item.route} onClick={() => nav(item.route)}
                                    ...
                                    onMouseEnter={e => { if (!active) { (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.03)'; (e.currentTarget as HTMLElement).style.color = theme.text; } }}
                                    onMouseLeave={e => { if (!active) { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.color = theme.textSecondary; } }}
                                >
        ...
        <>
            ...
            {mobileOpen && (
                <div onClick={onMobileClose} style={{
                    ...
                    <style>{`@media(max-width:768px){div[data-sidebar-overlay]{display:block!important}}`}</style>
                </div>
            )}

            {/* Mobile sidebar */}
            <div style={{
                position: 'fixed', top: 0, left: mobileOpen ? 0 : -260, width: 240,
                height: '100vh', zIndex: 50, transition: 'left 0.25s ease',
            }}>
                <style>{`@media(min-width:769px){div[data-sidebar-mobile]{display:none!important}}`}</style>
                {content}
            </div>

            {/* Desktop sidebar */}
            <div data-sidebar-desktop="" style={{ flexShrink: 0 }}>
                <style>{`@media(max-width:768px){div[data-sidebar-desktop]{display:none!important}}`}</style>
                {content}
            </div>
        </>
    );
