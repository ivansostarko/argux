    ...
    return (
        <div ref={ref} style={{ position: 'relative', flex: 1, minWidth: 140 }}>
            ...
            {open && <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: '#0d1220', border: `1px solid ${theme.border}`, borderRadius: 8, zIndex: 50, maxHeight: 220, display: 'flex', flexDirection: 'column', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
                <div style={{ padding: '6px 8px', borderBottom: `1px solid ${theme.border}`, display: 'flex', gap: 4 }}>
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search..." autoFocus style={{ flex: 1, padding: '5px 8px', background: theme.bgInput, color: theme.text, border: `1px solid ${theme.border}`, borderRadius: 4, fontSize: 11, fontFamily: 'inherit', outline: 'none' }} />
...
function ConfirmModal({ open, title, message, onConfirm, onCancel }: { open: boolean; title: string; message: string; onConfirm: () => void; onCancel: () => void }) {
    if (!open) return null;
    return (
        <div onClick={onCancel} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            <div onClick={e => e.stopPropagation()} style={{ background: '#0d1220', border: `1px solid ${theme.border}`, borderRadius: 14, width: '100%', maxWidth: 400, padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,0.6)', animation: 'argux-fadeIn 0.2s ease-out' }}>
                <div style={{ width: 48, height: 48, borderRadius: 12, background: theme.dangerDim, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', color: theme.danger }}>
                    <svg width="24" height="24" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M7.13 2.5a1 1 0 011.74 0l5.5 9.5A1 1 0 0113.5 13.5h-11a1 1 0 01-.87-1.5z"/><line x1="8" y1="6" x2="8" y2="8.5"/><circle cx="8" cy="10.5" r="0.5" fill="currentColor"/></svg>
                </div>
...
function TableSkeleton() {
    return (
        <div style={{ background: 'rgba(10,14,22,0.5)', border: `1px solid ${theme.border}`, borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '14px 16px', background: 'rgba(255,255,255,0.02)', borderBottom: `1px solid ${theme.border}` }}>
                <Skeleton height={12} width={600} />
            </div>
            {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} style={{ display: 'flex', gap: 16, padding: '14px 16px', borderBottom: `1px solid ${theme.border}20`, alignItems: 'center' }}>
                    <Skeleton width={36} height={36} radius={8} />
                    <div style={{ flex: 1 }}><Skeleton height={13} width={160} style={{ marginBottom: 6 }} /><Skeleton height={10} width={100} /></div>
                    <Skeleton height={12} width={80} />
                    <Skeleton height={12} width={100} />
                    <Skeleton height={12} width={60} />
                    <Skeleton height={20} width={50} radius={4} />
                    <div style={{ display: 'flex', gap: 4 }}><Skeleton width={28} height={28} radius={4} /><Skeleton width={28} height={28} radius={4} /><Skeleton width={28} height={28} radius={4} /></div>
                </div>
            ))}
        </div>
    );
...
function Tip({ text, children }: { text: string; children: React.ReactNode }) {
    ...
    return (
        <div style={{ position: 'relative', display: 'inline-flex' }} onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
            {children}
            {show && <div style={{ position: 'absolute', bottom: '100%', left: '50%', transform: 'translateX(-50%)', marginBottom: 6, background: '#1a1f2e', border: `1px solid ${theme.border}`, borderRadius: 6, padding: '4px 10px', fontSize: 11, color: theme.text, whiteSpace: 'nowrap' as const, zIndex: 40, boxShadow: '0 4px 12px rgba(0,0,0,0.3)', pointerEvents: 'none' as const }}>{text}</div>}
        </div>
    );
}
...
export default function PersonsIndex() {
    ...
    const filtered = persons.filter(p => {
        ...
        return ms && (!fName || `${p.firstName} ${p.lastName}`.toLowerCase().includes(fName.toLowerCase()))
            && (!fNick || p.nickname.toLowerCase().includes(fNick.toLowerCase()))
            && (!fEmail || p.email.toLowerCase().includes(fEmail.toLowerCase()))
            && (!fPhone || p.phone.includes(fPhone))
            && (!fGender || p.gender === fGender)
            && (!fDobFrom || p.dob >= fDobFrom) && (!fDobTo || p.dob <= fDobTo)
            && (!fTax || p.taxNumber.toLowerCase().includes(fTax.toLowerCase()))
            && (fNat.length === 0 || fNat.includes(p.nationality))
            && (fCountry.length === 0 || fCountry.includes(p.country))
            && (fLang.length === 0 || fLang.includes(p.language))
            && (fRisk.length === 0 || fRisk.includes(p.risk))
            && (fStatus.length === 0 || fStatus.includes(p.status))
            && (fReligion.length === 0 || fReligion.includes(p.religion));
    ...
    const handleDelete = () => {
        ...
        const p = persons.find(x => x.id === deleteTarget);
        setPersons(prev => prev.filter(x => x.id !== deleteTarget));
        setDeleteTarget(null);
        toast.success('Person deleted', `${p?.firstName} ${p?.lastName} removed from database.`);
    };
        ...
        <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, background: theme.bgInput, border: `1px solid ${theme.border}`, borderRadius: 8, padding: '0 14px' }}>
                    ...
                    <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search by name, email, phone, UUID..." style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', padding: '10px 0', color: theme.text, fontSize: 13, fontFamily: 'inherit' }} />
                </div>
                <button onClick={() => setShowFilters(!showFilters)} style={{ background: showFilters ? theme.accentDim : 'rgba(255,255,255,0.03)', border: `1px solid ${showFilters ? theme.accent : theme.border}`, borderRadius: 8, padding: '0 14px', cursor: 'pointer', color: showFilters ? theme.accent : theme.textSecondary, fontSize: 12, fontWeight: 600, fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><line x1="2" y1="4" x2="14" y2="4"/><line x1="4" y1="8" x2="12" y2="8"/><line x1="6" y1="12" x2="10" y2="12"/></svg>Filters
                    {hasAdv && <span style={{ background: theme.accent, color: '#fff', fontSize: 8, fontWeight: 700, padding: '1px 4px', borderRadius: 8 }}>!</span>}
                </button>
            </div>
            ...
            {showFilters && (
                <div style={{ background: theme.bgInput, border: `1px solid ${theme.border}`, borderRadius: 10, padding: 16, marginBottom: 16, animation: 'argux-fadeIn 0.2s ease-out' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 10, marginBottom: 10 }}>
                        {[['Name','fName',fName,setFName,'First or last name'],['Nickname','fNick',fNick,setFNick,'Alias'],['Email','fEmail',fEmail,setFEmail,'Email address'],['Phone','fPhone',fPhone,setFPhone,'Phone number'],['Tax Number','fTax',fTax,setFTax,'Tax ID']].map(([label,_k,val,setter,ph]) => (
            ...
            {loading ? <TableSkeleton /> : (
                <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' as const }}>
                    <div style={{ background: 'rgba(10,14,22,0.5)', border: `1px solid ${theme.border}`, borderRadius: 12, overflow: 'hidden', minWidth: 900 }}>
                        <style>{`.ptr{cursor:pointer;user-select:none} .ptr:hover{color:${theme.text}}`}</style>
                        {/* Header */}
                        <div style={{ display: 'grid', gridTemplateColumns: '80px 44px 1fr 90px 100px 90px 150px 130px 90px 65px 65px 90px', padding: '10px 12px', background: 'rgba(255,255,255,0.02)', borderBottom: `1px solid ${theme.border}`, fontSize: 9, fontWeight: 700, color: theme.textDim, letterSpacing: '0.08em', textTransform: 'uppercase' as const, alignItems: 'center', gap: 6 }}>
                            <span className="ptr" onClick={() => toggleSort('uuid')}>UUID<SI col="uuid"/></span>
                            <span></span>
                            <span className="ptr" onClick={() => toggleSort('lastName')}>Name<SI col="lastName"/></span>
                            <span>Nickname</span>
                            <span className="ptr" onClick={() => toggleSort('nationality')}>Nationality<SI col="nationality"/></span>
                            <span className="ptr" onClick={() => toggleSort('country')}>Country<SI col="country"/></span>
                            <span>Email</span>
                            <span>Phone</span>
                            <span>Tax No.</span>
                            <span className="ptr" onClick={() => toggleSort('status')}>Status<SI col="status"/></span>
                            <span className="ptr" onClick={() => toggleSort('risk')}>Risk<SI col="risk"/></span>
                            <span>Actions</span>
                        </div>
                        {paged.length === 0 ? (
                            <div style={{ padding: '48px 16px', textAlign: 'center' }}><div style={{ fontSize: 14, fontWeight: 600, color: theme.text, marginBottom: 4 }}>No persons found</div><div style={{ fontSize: 12, color: theme.textSecondary }}>Adjust search or filters.</div></div>
                        ) : paged.map((p, idx) => (
                            <div key={p.id} style={{ display: 'grid', gridTemplateColumns: '80px 44px 1fr 90px 100px 90px 150px 130px 90px 65px 65px 90px', padding: '8px 12px', alignItems: 'center', gap: 6, borderBottom: idx < paged.length-1 ? '1px solid rgba(255,255,255,0.03)' : 'none', fontSize: 11, transition: 'background 0.1s', cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.02)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')} onClick={() => router.visit(`/persons/${p.id}`)}>
                                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 9, color: theme.textDim, overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.uuid.slice(0, 8)}…</span>
                                ...
                                <span style={{ color: p.nickname ? theme.textSecondary : theme.textDim, fontStyle: p.nickname ? 'normal' : 'italic', fontSize: 11 }}>{p.nickname || '—'}</span>
                                <span style={{ color: theme.textSecondary, fontSize: 11 }}>{p.nationality}</span>
                                <span style={{ color: theme.textSecondary, fontSize: 11 }}>{p.country.length > 12 ? p.country.slice(0, 10) + '…' : p.country}</span>
