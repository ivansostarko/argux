...
export default function VehicleCreate() {
    ...
    const handleSave = () => {
        if (!plate) { toast.error('Validation', 'Registration plate is required.'); return; }
        setLoading(true);
        setTimeout(() => { setLoading(false); toast.success('Vehicle created', `${plate} added to database.`); router.visit('/vehicles'); }, 1200);
    };
    ...
    return (
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}><Button variant="secondary" onClick={() => router.visit('/vehicles')} style={{ width: 'auto', padding: '9px 18px', fontSize: 11 }}>Cancel</Button><Button onClick={handleSave} loading={loading} style={{ width: 'auto', padding: '9px 22px', fontSize: 11 }}>Create Vehicle</Button></div>
            </div>
            ...
            <div style={{ background: theme.bgInput, border: `1px solid ${theme.border}`, borderRadius: 12, padding: 20, marginBottom: 20 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 14 }}>
                    <div><Label>Type</Label><SearchSel value={type} onChange={setType} options={vehicleTypes} placeholder="Select type" /></div>
                    <div><Label>Make</Label><SearchSel value={make} onChange={setMake} options={vehicleMakes} placeholder="Select make" /></div>
                    <div><Label>Model</Label><input value={model} onChange={e => setModel(e.target.value)} placeholder="e.g. X5 M, A6" style={{ width: '100%', padding: '9px 12px', background: theme.bg, color: theme.text, border: `1px solid ${theme.border}`, borderRadius: 6, fontSize: 13, fontFamily: 'inherit', outline: 'none' }} /></div>
                    <div><Label>Year</Label><Sel value={year} onChange={setYear} options={vehicleYears} placeholder="Select year" /></div>
...
VehicleCreate.layout = (page: React.ReactNode) => <AppLayout>{page}</AppLayout>;
