...
function MS({ selected, onChange, options, placeholder }: { selected: string[]; onChange: (v: string[]) => void; options: { id: string; label: string }[]; placeholder: string }) {
    ...
    return (<div ref={ref} style={{ position: 'relative', flex: 1, minWidth: 100 }}><button onClick={() => { setOpen(!open); setQ(''); }} style={{ width: '100%', padding: '6px 8px', background: theme.bgInput, color: has ? theme.text : theme.textDim, border: `1px solid ${has ? theme.accent+'60' : theme.border}`, borderRadius: 5, fontSize: 10, fontFamily: 'inherit', cursor: 'pointer', textAlign: 'left' as const, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 3 }}><span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, flex: 1 }}>{has ? `${selected.length} sel.` : placeholder}</span>{has && <span style={{ background: theme.accentDim, color: theme.accent, fontSize: 8, fontWeight: 700, padding: '0px 3px', borderRadius: 3 }}>{selected.length}</span>}</button>{open && <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: 4, background: '#0d1220', border: `1px solid ${theme.border}`, borderRadius: 8, zIndex: 60, maxHeight: 180, display: 'flex', flexDirection: 'column', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}><div style={{ padding: '4px 6px', borderBottom: `1px solid ${theme.border}`, display: 'flex', gap: 4 }}><input value={q} onChange={e => setQ(e.target.value)} placeholder="Search..." autoFocus style={{ flex: 1, padding: '4px 6px', background: theme.bgInput, color: theme.text, border: `1px solid ${theme.border}`, borderRadius: 3, fontSize: 10, fontFamily: 'inherit', outline: 'none' }} />{has && <button onClick={() => onChange([])} style={{ background: 'none', border: 'none', color: theme.danger, fontSize: 9, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>Clear</button>}</div><div style={{ overflowY: 'auto', flex: 1 }}>{filtered.map(o => { const c = selected.includes(o.id); return <div key={o.id} onClick={() => toggle(o.id)} style={{ padding: '4px 8px', cursor: 'pointer', fontSize: 10, color: c ? theme.accent : theme.text, display: 'flex', alignItems: 'center', gap: 5 }} onMouseEnter={e => (e.currentTarget.style.background = 'rgba(128,128,128,0.08)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}><div style={{ width: 10, height: 10, borderRadius: 2, border: `1.5px solid ${c ? theme.accent : theme.border}`, background: c ? theme.accent : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{c && <svg width="6" height="6" viewBox="0 0 10 10" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round"><polyline points="2,5 4.5,7.5 8,3"/></svg>}</div>{o.label}</div>; })}</div></div>}</div>);
}
...
function Md({ text }: { text: string }) {
    const html = text
        ...
        .replace(/\|(.+)\|/g, (m) => { const cells = m.split('|').filter(Boolean).map(c => c.trim()); return `<tr>${cells.map(c => c.match(/^-+$/) ? '' : `<td>${c}</td>`).join('')}</tr>`; })
        .replace(/\n/g, '<br/>');
...
export default function ChatIndex() {
    ...
    const handleNewConv = () => {
        const conv: Conversation = { id: generateConvId(), title: 'New Conversation', personIds: [], orgIds: [], messages: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    ...
    const handleDelete = () => {
        ...
        if (activeId === deleteId) setActiveId(conversations.find(c => c.id !== deleteId)?.id || null);
        setDeleteId(null);
        toast.warning('Conversation deleted');
    };
    ...
    const handleSend = () => {
        ...
        setTyping(true);
        setTimeout(() => {
            const aiMsg: ChatMessage = { id: generateMsgId(), role: 'assistant', content: getAIResponse(), attachments: [], timestamp: new Date().toISOString() };
            setConversations(prev => prev.map(c => c.id === activeId ? { ...c, messages: [...c.messages, aiMsg], updatedAt: new Date().toISOString() } : c));
            setTyping(false);
            scrollBottom();
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        Array.from(files).forEach(f => {
            ...
            if (['mp4','avi','mov','mkv','webm'].includes(ext)) type = 'video';
            if (['mp3','wav','ogg','m4a','flac'].includes(ext)) type = 'audio';
            if (['heic','raw','cr2','nef'].includes(ext)) type = 'photo';
            const size = f.size > 1048576 ? `${(f.size / 1048576).toFixed(1)} MB` : `${Math.round(f.size / 1024)} KB`;
            setPendingFiles(prev => [...prev, { id: `file-${Date.now()}-${Math.random().toString(36).slice(2,5)}`, name: f.name, type, size }]);
        });
        if (fileRef.current) fileRef.current.value = '';
        toast.info('File attached', `${files.length} file(s)`);
    };

    const handleKeyDown = (e: React.KeyboardEvent) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } };

    const handleEntityChange = (field: 'personIds' | 'orgIds', val: string[]) => {
        if (!activeId) return;
        setConversations(prev => prev.map(c => c.id === activeId ? { ...c, [field]: val } : c));
    };
    ...
    return (
        <div className="chat-page">
            ...
            {/* Mobile overlay */}
            {sidebarOpen && <div onClick={() => setSidebarOpen(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 99, display: 'none' }} className="chat-mobile-overlay" />}
                ...
                <div className="chat-sidebar-list">
                    {filteredConvs.length === 0 ? <div style={{ padding: '20px 12px', textAlign: 'center', fontSize: 12, color: theme.textDim }}>No conversations</div> : filteredConvs.map(c => (
                        <div key={c.id} className={`chat-conv-item ${activeId === c.id ? 'active' : ''}`} onClick={() => { setActiveId(c.id); setSidebarOpen(false); }}>
                            ...
                            {(c.personIds.length > 0 || c.orgIds.length > 0) && <div className="chat-entity-tags" style={{ marginTop: 4 }}>
                                {c.personIds.slice(0, 2).map(id => { const n = allNodes.find(x => x.id === id); return n ? <span key={id} className="chat-entity-tag" style={{ background: theme.accentDim, color: theme.accent, border: `1px solid ${theme.accent}30` }}>{n.label.split(' ')[0]}</span> : null; })}
                                {c.orgIds.slice(0, 1).map(id => { const n = allNodes.find(x => x.id === id); return n ? <span key={id} className="chat-entity-tag" style={{ background: 'rgba(139,92,246,0.08)', color: '#8b5cf6', border: '1px solid rgba(139,92,246,0.2)' }}>{n.label.split(' ')[0]}</span> : null; })}
                                {(c.personIds.length + c.orgIds.length) > 3 && <span className="chat-entity-tag" style={{ background: 'rgba(255,255,255,0.04)', color: theme.textDim }}>+{c.personIds.length + c.orgIds.length - 3}</span>}
                            </div>}
                ...
                {activeConv ? <>
                    ...
                    <div className="chat-header">
                        <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
                            <button className="chat-mobile-toggle chat-input-btn" onClick={() => setSidebarOpen(true)}><svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><line x1="2" y1="4" x2="14" y2="4"/><line x1="2" y1="8" x2="14" y2="8"/><line x1="2" y1="12" x2="14" y2="12"/></svg></button>
                            <div style={{ minWidth: 0 }}>
                                <div className="chat-header-title">{activeConv.title}</div>
                                <div className="chat-header-meta">{activeConv.messages.length} messages · {new Date(activeConv.updatedAt).toLocaleString()}</div>
                            </div>
                        </div>
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexShrink: 0 }}>
                            ...
                            <MS selected={activeConv.orgIds} onChange={v => handleEntityChange('orgIds', v)} options={orgOpts} placeholder="Organizations" />
                        </div>
                    </div>
                    ...
                    <div className="chat-messages">
                        {activeConv.messages.length === 0 && !typing && (
                            <div className="chat-empty">
                                ...
                                <div style={{ fontSize: 13, color: theme.textSecondary, textAlign: 'center', maxWidth: 400 }}>Ask questions about persons, organizations, vehicles, connections, risk assessments, or upload files for analysis.</div>
                            </div>
                        )}
                        {activeConv.messages.map(msg => (
                            <div key={msg.id} className={`chat-msg ${msg.role}`}>
                                <div className="chat-msg-avatar" style={{ background: msg.role === 'user' ? theme.accent : `linear-gradient(135deg, #1e3a5f, #2d5a27)` }}>
                                    {msg.role === 'user' ? <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="8" cy="5" r="3"/><path d="M2.5 14.5c0-3 2.5-5 5.5-5s5.5 2 5.5 5"/></svg> : <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><circle cx="8" cy="8" r="6"/><path d="M8 5v3l2 2"/></svg>}
                                </div>
                                <div className="chat-msg-body">
                                    ...
                                    {msg.attachments.length > 0 && <div className="chat-attachments">
                                        {msg.attachments.map(a => a.url && (a.type === 'image' || a.type === 'photo') ? (
                                            <div key={a.id} className="chat-attach-img" onClick={() => setLightboxUrl(a.url!)}><img src={a.url} alt={a.name} /></div>
                                        ) : (
                                            <div key={a.id} className="chat-attach-chip"><AttachIcon type={a.type} /><span>{a.name}</span><span className="size">({a.size})</span></div>
                                        ))}
                                    </div>}
                    ...
                    <div className="chat-input-area">
                        <div className="chat-input-row">
                            <div className="chat-input-wrap">
                                ...
                                <textarea ref={textareaRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Ask ARGUX AI anything..." rows={1} />
