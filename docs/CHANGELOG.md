...
| Method | Endpoint | Purpose |
|---|---|---|
| GET | `/mock-api/notifications` | 20 notifications (filter severity/type/unread + counts) |
| PATCH | `/mock-api/notifications/{id}/read` | Toggle read/unread |
| POST | `/mock-api/notifications/read-all` | Mark all as read (returns count) |

- Original 269-line page design fully preserved (tabs, severity badges, source labels)
- Only 25 lines added: API helpers + useEffect fetch + async handlers
- 20 notifications: 5 unread, 4 critical, 6 warning, 10 info
- 6 types: system, storage, user, security, device, backup
- Unit tests: 10 tests (list, filter severity/type/unread, counts, toggle read/unread, 404, read-all)
